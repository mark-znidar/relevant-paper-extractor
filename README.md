...
**Each generated prompt file is tokenized using the OpenAI tokenizer (tiktoken), and the total token count is included in the output filename** — making it easy to verify which files fit within a given model's context window.
...
**Token counting:** The total token count is computed using the OpenAI tokenizer (tiktoken) and encoded directly into the output filename, e.g. `…_993847tok.txt`.
